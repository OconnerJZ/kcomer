...
export const useBusinessMenu = (businessId) => {
  ...
  const createItem = useCallback(
    async (itemData, imageFile = null) => {
      ...
      try {
        ...
        if (imageFile) {
          const uploadResult = await uploadHelpers.uploadImage(
            imageFile,
            uploadImage,
          );
        ...
        const errorMessage =
          err?.data?.message || err?.message || "Error al crear item";
  ...
  const updateItem = useCallback(
    async (itemId, itemData, imageFile = null) => {
      ...
      try {
        ...
        const errorMessage =
          err?.data?.message || err?.message || "Error al actualizar item";
        setError(errorMessage);
        return { success: false, error: errorMessage };
      }
    },
  ...
  const deleteItem = useCallback(
    async (itemId) => {
      ...
      try {
        ...
        const errorMessage =
          err?.data?.message || err?.message || "Error al eliminar item";
        setError(errorMessage);
        return { success: false, error: errorMessage };
      }
    },
  ...
  const selectors = useMemo(
    () => ({
      ...
      getAvailableItems: () =>
        menu.filter((item) => item.is_available || item.available),
  ...
  return {
    menu,
    loading:
      loading || creating || updating || deleting || toggling || uploading,
