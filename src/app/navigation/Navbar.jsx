  ...
  return (
    <Box>
            ...
            {isAuthenticated ? (
              <>
                ...
                <Menu anchorEl={anchorEl} open={menuOpen} onClose={() => setAnchorEl(null)} transformOrigin={{ horizontal: "right", vertical: "top" }} anchorOrigin={{ horizontal: "right", vertical: "bottom" }} PaperProps={{ sx: { minWidth: 200, mt: 1 } }}>
                  ...
                  {user?.role === "owner" && <MenuItem onClick={() => goFromMenu("/owner")}><ListItemIcon><Dashboard fontSize="small" /></ListItemIcon><ListItemText>Panel de Negocio</ListItemText></MenuItem>}
                  <Divider />
      ...
      <Box sx={{ width: "100%", height: "9%", position: "fixed", bottom: 0, left: 0, zIndex: 1000, display: { xs: "flex", sm: "none" }, borderTop: "1px solid #e0e0e0", backgroundColor: "#fff" }}>
        <BottomNavigation showLabels value={value} onChange={(_, newValue) => { const toGo = navItems[newValue]?.link ?? "/perfil"; setValue(newValue); navigate(toGo); }} sx={{ width: "100%", "& .MuiBottomNavigationAction-root": { minWidth: "auto" } }}>
          {navItems.map((item) => (
            <BottomNavigationAction key={item.title} label={item.title === "Registro" && isAuthenticated ? "Negocio" : item.title} icon={<Badge badgeContent={item.link === "orden" ? cartCount : 0} color="error">{item.icon}</Badge>} />
          ))}
