      ...
      <Box onClick={expandCard} sx={{ cursor: "pointer" }}>
        <Box sx={{ position: "relative", height: 164, m: 1.15, mb: 0, borderRadius: 4, overflow: "hidden", backgroundImage: coverUrl ? `url(${coverUrl})` : "linear-gradient(135deg,#ede7e3,#d8cec8)", backgroundSize: "cover", backgroundPosition: "center" }}>
          ...
          <Stack direction="row" justifyContent="space-between" sx={{ position: "absolute", top: 11, left: 11, right: 11 }}>
            ...
            <IconButton size="small" onClick={(e) => { e.stopPropagation(); setScheduleOpen(true); }} sx={{ width: 31, height: 31, bgcolor: "rgba(255,255,255,.92)", "&:hover": { bgcolor: "white" } }}><AccessTimeRounded sx={{ fontSize: 17 }} /></IconButton>
          </Stack>
          <Box sx={{ position: "absolute", left: 14, right: 14, bottom: 12 }}><Stack direction="row" spacing={1.1} alignItems="flex-end"><Avatar src={logoUrl} sx={{ width: 48, height: 48, border: "2px solid white", boxShadow: "0 5px 16px rgba(0,0,0,.2)" }}>{business.name?.charAt(0)}</Avatar><Box minWidth={0} flex={1}><Typography noWrap sx={{ color: "white", fontSize: "1.18rem", fontWeight: 900, lineHeight: 1.05, textShadow: "0 2px 10px rgba(0,0,0,.2)" }}>{business.name || "Negocio"}</Typography><Stack direction="row" spacing={.8} alignItems="center" sx={{ mt: .45, color: "rgba(255,255,255,.86)" }}>{business.location?.city && <><PlaceRounded sx={{ fontSize: 13 }} /><Typography variant="caption" fontWeight={650}>{business.location.city}</Typography></>}{business.hasDelivery && <><DeliveryDiningRounded sx={{ fontSize: 14 }} /><Typography variant="caption">Delivery</Typography></>}</Stack></Box></Stack></Box>
        </Box>
        <Box sx={{ px: 2, pt: 1.45, pb: 1.25 }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Box minWidth={0}><Typography variant="caption" color="text.secondary" sx={{ display: "block" }}>{business.description ? business.description.slice(0, 54) : "Descubre su menú, ubicación y experiencia."}{business.description?.length > 54 ? "…" : ""}</Typography></Box><Stack direction="row" alignItems="center" sx={{ ml: 1 }}><Typography variant="caption" fontWeight={800}>{expanded ? "Ocultar" : "Explorar"}</Typography><KeyboardArrowDownRounded sx={{ transform: expanded ? "rotate(180deg)" : "none", transition: ".2s" }} /></Stack></Stack></Box>
      </Box>
      <Collapse in={expanded} timeout="auto" unmountOnExit><Box sx={{ borderTop: "1px solid rgba(35,29,26,.06)", bgcolor: "#fff" }}><CardPlaceFront flipped={flipped} onMovement={onMovement} data={business} />{movement && <MovementContent movement={movement} flipped={flipped} onMovement={onMovement} business={business} />}</Box></Collapse>
    </StyledCard>
