...
const DeliveryInfo = ({ order }) => {
  ...
  return (
      <Box sx={{ height: "100%", border: "1px solid", borderColor: "divider", borderRadius: "8px", overflow: "hidden", bgcolor: "rgba(255,255,255,.72)" }}>
        <Box sx={{ px: 2, pt: 2 }}>
          <Typography variant="overline" color="text.secondary" sx={{ letterSpacing: ".13em", fontSize: ".64rem" }}>ENTREGA</Typography>
          <Typography variant="subtitle1" fontWeight={600}>{isDelivery ? "Dirección del cliente" : "Recoger en tienda"}</Typography>
        </Box>

        {isDelivery && (
          <Box sx={{ display: "flex", gap: .5, mx: 2, mt: 1.5, p: .4, bgcolor: "action.hover", borderRadius: "8px" }}>
            {[{ key: "address", label: "Dirección", icon: LocationOn }, { key: "map", label: "Mapa", icon: MapRounded }].map(({ key, label, icon: Icon }) => (
              <ButtonBase key={key} onClick={() => setView(key)} sx={{ flex: 1, py: .8, borderRadius: "8px", bgcolor: view === key ? "background.paper" : "transparent", boxShadow: view === key ? "0 4px 12px rgba(0,0,0,.06)" : 0 }}>
        ...
        <Box sx={{ p: 2 }}>
          {!isDelivery || view === "address" ? (
            <Stack direction="row" spacing={1.2} alignItems="flex-start">
              ...
              <Box>
                <Typography variant="body2" fontWeight={600} sx={{ lineHeight: 1.6 }}>{deliveryAddress}</Typography>
            ...
            <Box sx={{ height: 230, borderRadius: "8px", overflow: "hidden", border: "1px solid", borderColor: "divider" }}>
        ...
        {order.notes && <Box sx={{ borderTop: "1px solid", borderColor: "divider", p: 2, bgcolor: "rgba(168,117,60,.06)" }}><Stack direction="row" spacing={1}><StickyNote2 sx={{ fontSize: 18, color: "secondary.dark" }} /><Box><Typography variant="caption" color="text.secondary" fontWeight={600}>NOTAS DEL CLIENTE</Typography><Typography variant="body2" sx={{ mt: .35 }}>{order.notes}</Typography></Box></Stack></Box>}
      </Box>
  );
};
