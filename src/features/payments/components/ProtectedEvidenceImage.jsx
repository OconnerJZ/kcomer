...
export default function ProtectedEvidenceImage({ evidence, size = 64, onOpen }) {
  ...
  return <Box component="button" type="button" onClick={() => src && onOpen?.(src)} aria-label="ampliar comprobante" sx={{ width: size, height: size, p: 0, border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden", cursor: src ? "zoom-in" : "default", bgcolor: "grey.100", display: "grid", placeItems: "center" }}>{src ? <Box component="img" src={src} alt="Comprobante protegido" sx={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <CircularProgress size={20} />}</Box>;
}
