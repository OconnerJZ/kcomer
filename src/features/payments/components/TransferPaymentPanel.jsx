...
export default function TransferPaymentPanel({ order }) {
  ...
  return (
    <Box sx={{ mt: 2, pt: 2, borderTop: "1px solid", borderColor: "divider" }}>
      ...
      {payment && <Alert severity={statusCopy[payment.status]?.severity || "info"} sx={{ mt: 1.5, borderRadius: "10px" }}>{statusCopy[payment.status]?.text}{payment.ownerMessage && <Typography variant="body2" fontWeight={700} sx={{ mt: .5 }}>{payment.ownerMessage}</Typography>}</Alert>}
