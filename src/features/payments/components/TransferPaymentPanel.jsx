  ...
  return (
    <Box sx={{ mt: 2, pt: 2, borderTop: "1px solid", borderColor: "divider" }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.5} gap={1}>
        <Box><Typography variant="subtitle2" fontWeight={850}>Pago por transferencia</Typography><Typography variant="caption" color="text.secondary">Tu orden continúa su proceso aunque la revisión esté pendiente.</Typography></Box>
        {payment?.status === "reviewed" && <Chip size="small" color="success" icon={<VerifiedOutlined />} label="Revisado" />}
      </Stack>

      {bankCollapsed ? <Accordion elevation={0} disableGutters sx={{ border: "1px solid", borderColor: "divider", borderRadius: "12px !important", "&:before": { display: "none" } }}><AccordionSummary expandIcon={<ExpandMore />}><Typography variant="body2" fontWeight={750}>Ver datos bancarios</Typography></AccordionSummary><AccordionDetails>{bankContent}</AccordionDetails></Accordion> : bankContent}

      {payment && <Alert severity={statusCopy[payment.status]?.severity || "info"} sx={{ mt: 1.5, borderRadius: 2 }}>{statusCopy[payment.status]?.text}{payment.ownerMessage && <Typography variant="body2" fontWeight={700} sx={{ mt: .5 }}>{payment.ownerMessage}</Typography>}</Alert>}
      ...
      <Box onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); selectFile(event.dataTransfer.files?.[0]); }} onClick={() => inputRef.current?.click()} sx={{ mt: 1.5, p: 2, border: "1.5px dashed", borderColor: file ? "primary.main" : "divider", borderRadius: 2.5, textAlign: "center", cursor: "pointer", bgcolor: file ? "rgba(255,75,69,.035)" : "grey.50" }}>
        ...
        {preview ? <Stack direction="row" spacing={1.5} alignItems="center"><Box component="img" src={preview} alt="Vista previa" sx={{ width: 64, height: 64, objectFit: "cover", borderRadius: 2 }} /><Box textAlign="left" minWidth={0}><Typography variant="body2" fontWeight={800} noWrap>{file.name}</Typography><Typography variant="caption" color="text.secondary">Toca para cambiar la imagen</Typography></Box></Stack> : <><CloudUpload color="action" /><Typography variant="body2" fontWeight={800}>Arrastra o selecciona tu comprobante</Typography><Typography variant="caption" color="text.secondary">JPG, PNG o WebP · máximo 5 MB</Typography></>}
      </Box>
      {file && <Stack direction="row" spacing={1} mt={1}><Button variant="outlined" fullWidth onClick={() => setViewer(preview)} startIcon={<ImageOutlined />} sx={{ textTransform: "none" }}>Vista previa</Button><Button variant="contained" fullWidth disabled={isLoading} onClick={send} sx={{ textTransform: "none" }}>{isLoading ? <CircularProgress size={20} color="inherit" /> : payment ? "Enviar nueva evidencia" : "Reportar pago"}</Button></Stack>}
