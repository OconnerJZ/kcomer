...
export default function BusinessPlanTab({ businessId }) {
  ...
  const current = {
    ...legacyCurrent,
    ...(data.plan || {}),
    adsEnabled: data.policies?.adsEnabled ?? legacyCurrent.adsEnabled,
  };
  ...
  return <Stack gap={2.5}>
    ...
    <Paper variant="outlined" sx={{ p: 2.5, borderRadius: "8px" }}><Typography variant="h6" fontWeight={600}>Uso y límites</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Los límites comerciales describen escala del negocio. Realtime, órdenes y Shared Orders no se bloquean por nivel.</Typography><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2,minmax(0,1fr))", lg: "repeat(3,minmax(0,1fr))" }, gap: 2 }}>{Object.entries(data.limits || {}).map(([key, value]) => { const progress = limitProgress(value); return <Box key={key} sx={{ p: 1.7, bgcolor: "grey.50", borderRadius: "8px" }}><Typography variant="body2" fontWeight={600}>{LIMIT_LABELS[key] || key}</Typography><Typography variant="caption" color="text.secondary">{formatPlanLimit(value)}</Typography>{progress != null && <LinearProgress variant="determinate" value={progress} sx={{ mt: 1, height: 6, borderRadius: "8px" }}/>}</Box>; })}</Box></Paper>
    <Box><Typography variant="h6" fontWeight={600}>Comparar niveles</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>La estructura está preparada sin inventar precios ni límites todavía. Las diferencias comerciales se activarán conforme aprobemos capacidad y herramientas reales.</Typography><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2,minmax(0,1fr))", xl: "repeat(4,minmax(0,1fr))" }, gap: 1.5 }}>{(data.catalog || []).map((plan) => <PlanCard key={plan.code} plan={plan} current={plan.code === current.code}/>)}</Box></Box>
  </Stack>;
}
