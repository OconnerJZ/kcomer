...
export default function SharedOrderPage() {
  ...
  useEffect(() => {
    ...
    joinToken(token).unwrap().then((joined) => { setSessionId(joined.id); navigate(`/orden-compartida/${joined.id}`, { replace: true }); }).catch((requestError) => setError(sharedOrderError(requestError, "El enlace ya no es válido")));
  ...
  if (!sessionId || isLoading) return <GeneralContent title="Orden compartida"><Box sx={{ py: 8, textAlign: "center" }}><CircularProgress /></Box></GeneralContent>;
  if (!session) return <GeneralContent title="Orden compartida"><Alert severity="error">{error || "No se pudo abrir la orden compartida"}</Alert></GeneralContent>;

  return <GeneralContent title={session.title}>
    <Box sx={{ maxWidth: 920, mx: "auto", px: 2, pb: 6 }}><Stack gap={2}>
      ...
      <Paper sx={{ p: 2.5, borderRadius: 3 }}><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={2}>
        ...
        {session.status === "open" && <Button startIcon={session.isHost ? <Cancel /> : <ExitToApp />} color="inherit" onClick={() => act(() => session.isHost ? cancel({ id: session.id, expectedVersion: session.version }).unwrap() : leave({ id: session.id, expectedVersion: session.version }).unwrap()).then((result) => result && navigate("/orden"))}>{session.isHost ? "Cancelar" : "Salir"}</Button>}
      </Stack></Paper>
      ...
      {session.status === "open" && <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 3 }}><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ sm: "center" }} gap={1}><Box><Typography variant="h6">Agregar mi carrito</Typography><Typography color="text.secondary">{cartItems.length ? `${cartItems.length} producto(s) listos para agregar` : "Ve a Explorar, arma tu carrito y vuelve aquí."}</Typography></Box><Button variant="contained" startIcon={<AddShoppingCart />} disabled={!cartItems.length || addState.isLoading} onClick={importCart}>Agregar</Button></Stack></Paper>}
