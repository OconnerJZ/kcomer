  ...
  return <SharedOrderShell embedded={embedded} title={session.title}>
    <Box sx={{ maxWidth: 920, mx: "auto", px: { xs: 1.5, sm: 2.5 }, py: { xs: 2, sm: 3 }, pb: { xs: 4, sm: 6 } }}><Stack gap={{ xs: 1.5, sm: 2 }}>
      ...
      <Box sx={{ pb: 1.5, borderBottom: "1px solid", borderColor: "divider", textAlign: "center" }}>
        ...
        {session.status === "open" && <Button fullWidth startIcon={session.isHost ? <Cancel /> : <ExitToApp />} onClick={exitSession} sx={{ mt: 1.5, color: "error.dark", bgcolor: "rgba(217,58,58,.07)", borderRadius: 2, fontWeight: 800, "&:hover": { bgcolor: "rgba(217,58,58,.12)" } }}>{session.isHost ? "Cerrar grupo" : "Salir del grupo"}</Button>}
      </Box>
