  ...
  return <SharedOrderShell embedded={embedded} title={session.title}>
    <Box sx={{ maxWidth: 920, mx: "auto", px: { xs: 1.5, sm: 2.5 }, py: { xs: 2, sm: 3 }, pb: { xs: 4, sm: 6 } }}><Stack gap={{ xs: 1.5, sm: 2 }}>
      ...
      <Box sx={{ pb: 1.5, borderBottom: "1px solid", borderColor: "divider", textAlign: "center" }}>
        <Typography variant="h5" fontWeight={700}>{session.title}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.45 }}>{session.participants.length} {session.participants.length === 1 ? "participante" : "participantes"}</Typography>
        {session.status === "open" && <Button fullWidth startIcon={session.isHost ? <Cancel /> : <ExitToApp />} onClick={exitSession} sx={{ mt: 1.5, color: "error.dark", bgcolor: "rgba(217,58,58,.07)", borderRadius: "8px", fontWeight: 600, "&:hover": { bgcolor: "rgba(217,58,58,.12)" } }}>{session.isHost ? "Cerrar grupo" : "Salir del grupo"}</Button>}
      </Box>
      ...
      {session.status === "open" && <Box sx={{ px: { xs: 1.5, sm: 2 }, py: 1.5, borderLeft: "4px solid", borderColor: "secondary.main", bgcolor: "rgba(168,117,60,.07)", borderRadius: "0 12px 12px 0" }}><Typography variant="subtitle1" fontWeight={600}>Agrega desde el menú</Typography><Typography variant="body2" color="text.secondary">En Explorar, cada producto se añadirá directamente a tu pedido dentro del grupo. Si prefieres uno aparte, cambia a “Pedido individual”.</Typography></Box>}
