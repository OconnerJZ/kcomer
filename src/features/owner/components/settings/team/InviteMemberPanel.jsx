import { Button, CircularProgress, MenuItem, Paper, Select, Stack, TextField, Typography } from "@mui/material";
...
export default function InviteMemberPanel({ email, role, loading, onEmailChange, onRoleChange, onInvite }) {
  return <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 3 }}><Stack spacing={2}>
    ...
    <Stack direction={{ xs: "column", md: "row" }} spacing={1.5}>
      ...
      <Select value={role} onChange={(event) => onRoleChange(event.target.value)} sx={{ minWidth: 180 }}>{BUSINESS_TEAM_ROLES.map((item) => <MenuItem key={item.value} value={item.value}>{item.label}</MenuItem>)}</Select>
