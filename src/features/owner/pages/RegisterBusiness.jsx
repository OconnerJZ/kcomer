...
import { Alert, Box, Button, Card, CardContent, Fade, LinearProgress, Paper, Snackbar, Stack, Typography } from "@mui/material";
...
const RegisterBusiness = ({ onSuccess }) => {
  ...
  const validateForm = () => {
    let valid = true; const nextErrors = {};
    steps[currentTab].fields.forEach((field) => {
      if (!field.required) return;
      const value = formValues[field.name];
      const empty = field.type === "autocomplete-multiple" || field.type === "schedule" ? !Array.isArray(value) || !value.length : field.type === "map" ? !value?.latitude || !value?.longitude : !value || (typeof value === "string" && !value.trim());
  ...
  const handleSubmit = async () => {
    try {
      ...
      const ready = ["owner", "admin"].includes(refreshedUser?.role);
      setOwnerReady(ready); setSubmitted(true);
  ...
  if (submitted) return <GeneralContent><Box sx={{ minHeight: 520, display: "grid", placeItems: "center", px: 2 }}><Card elevation={0} sx={{ maxWidth: 520, borderRadius: 5, border: "1px solid", borderColor: "divider" }}><CardContent><Result status="success" title="¡Tu negocio ya está en Kcomer!" subTitle="Ahora puedes completar su menú, portada y configuración desde el panel." extra={<Button variant="contained" onClick={() => navigate(ownerReady ? "/owner" : "/explorar")}>{ownerReady ? "Administrar negocio" : "Continuar"}</Button>} /></CardContent></Card></Box></GeneralContent>;

  const step = steps[currentTab];
  return (
    <GeneralContent>
      <Box sx={{ minHeight: "calc(100vh - 64px)", py: { xs: 3, md: 6 }, px: 2, backgroundImage: `linear-gradient(rgba(255,255,255,.78),rgba(255,255,255,.9)), url(${Bg})`, backgroundSize: "cover", backgroundPosition: "center" }}>
        <Paper elevation={0} sx={{ mx: "auto", width: "100%", maxWidth: 720, overflow: "hidden", borderRadius: { xs: 4, md: 6 }, border: "1px solid rgba(255,255,255,.9)", boxShadow: "0 24px 70px rgba(32,28,26,.12)", bgcolor: "rgba(255,255,255,.88)", backdropFilter: "blur(18px)" }}>
          <Box sx={{ p: { xs: 2.5, sm: 4 }, pb: 2, background: "linear-gradient(135deg,#201c1b 0%,#302724 100%)", color: "white" }}>
            <Stack direction="row" spacing={2} alignItems="center"><Box sx={{ width: 48, height: 48, borderRadius: 3, display: "grid", placeItems: "center", bgcolor: "rgba(255,255,255,.1)" }}><StorefrontRounded /></Box><Box><Typography variant="overline" sx={{ opacity: .65, letterSpacing: ".14em" }}>CREAR NEGOCIO</Typography><Typography variant="h4" fontWeight={900} sx={{ letterSpacing: "-.035em" }}>Hazlo fácil de descubrir.</Typography></Box></Stack>
            <Typography variant="body2" sx={{ mt: 2, maxWidth: 560, opacity: .72 }}>Tres pasos claros. Después podrás afinar menú, fotografías, pagos y delivery desde tu panel.</Typography>
          </Box>
          <LinearProgress variant="determinate" value={((currentTab + 1) / steps.length) * 100} sx={{ height: 4 }} />
          <Box sx={{ p: { xs: 2.5, sm: 4 } }}>
            <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={2} sx={{ mb: 3 }}><Box><Typography variant="caption" color="primary" fontWeight={800}>PASO {currentTab + 1} / {steps.length}</Typography><Typography variant="h5" fontWeight={850}>{step.label}</Typography><Typography variant="body2" color="text.secondary">{step.subtitle}</Typography></Box></Stack>
            <Fade in key={currentTab} timeout={280}><Box>{loadingFoodTypes && currentTab === 0 ? <Typography color="text.secondary">Cargando tipos de comida...</Typography> : step.fields.map((field) => <FormField key={field.name} field={field} formValues={formValues} setFormValues={setFormValues} error={errors[field.name]} />)}</Box></Fade>
