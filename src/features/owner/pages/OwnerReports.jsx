...
export default function OwnerReports({ businessId }) {
  ...
  return <Box sx={{ pb: 3, opacity: isFetching ? .78 : 1, transition: "opacity .2s" }}>
    <Box sx={{ color: "white", bgcolor: "#25272b", backgroundImage: "radial-gradient(circle at 86% 0%, rgba(255,75,69,.3), transparent 34%), linear-gradient(135deg,#222428,#303338)", borderRadius: { xs: 3, sm: 4 }, p: { xs: 2.2, sm: 3 }, boxShadow: "0 24px 55px rgba(26,28,31,.16)" }}>
      <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={2} sx={{ mb: 3 }}><Box><Stack direction="row" alignItems="center" gap={.8}><TrendingUpRounded sx={{ color: "#ff6a64", fontSize: 20 }}/><Typography variant="overline" sx={{ color: "rgba(255,255,255,.62)", letterSpacing: ".14em" }}>Inteligencia del negocio</Typography></Stack><Typography variant="h4" fontWeight={900} sx={{ letterSpacing: "-.04em" }}>Rendimiento</Typography><Typography variant="body2" sx={{ color: "rgba(255,255,255,.67)", mt: .6 }}>Finanzas, clientes, productos y operación comparados con el periodo anterior.</Typography></Box><FormControl size="small" sx={{ minWidth: { xs: "100%", sm: 180 } }}><Select value={period} onChange={(event) => setPeriod(event.target.value)} sx={{ color: "white", bgcolor: "rgba(255,255,255,.08)", borderRadius: 2.3, ".MuiOutlinedInput-notchedOutline": { borderColor: "rgba(255,255,255,.15)" }, ".MuiSvgIcon-root": { color: "white" } }}>{periods.map((option) => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}</Select></FormControl></Stack>
