...
export default function FinancialOverview({ salesByDay, paymentMix, orderTypeMix, categoryPerformance }) {
  return <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "minmax(0,2fr) minmax(280px,1fr)" }, gap: 2 }}>
    <ReportPanel title="Pulso de ventas" subtitle="Ventas brutas y ticket promedio de órdenes completadas">
      {!salesByDay.length ? <EmptyReport /> : <Box sx={{ height: { xs: 250, sm: 310 } }}><ResponsiveContainer><AreaChart data={salesByDay} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}><defs><linearGradient id="financeRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#c65a50" stopOpacity={.08}/><stop offset="95%" stopColor="#c65a50" stopOpacity={.08}/></linearGradient></defs><CartesianGrid strokeDasharray="3 6" vertical={false} stroke="#e5e0d9"/><XAxis dataKey="label" axisLine={false} tickLine={false} minTickGap={25} tick={{ fontSize: 10 }}/><YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10 }}/><Tooltip formatter={(value, name) => [money(value), name]}/><Area type="monotone" dataKey="revenue" name="Ventas" stroke="#c65a50" strokeWidth={2.4} fill="url(#financeRevenue)"/><Area type="monotone" dataKey="averageTicket" name="Ticket promedio" stroke="#3c4148" strokeWidth={1.8} fill="transparent"/></AreaChart></ResponsiveContainer></Box>}
    </ReportPanel>
    <Stack gap={2}>
