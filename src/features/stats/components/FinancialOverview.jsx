...
export default function FinancialOverview({ salesByDay, paymentMix, orderTypeMix, categoryPerformance }) {
  return <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "minmax(0,2fr) minmax(280px,1fr)" }, gap: 2 }}>
    <ReportPanel title="Pulso de ventas" subtitle="Ventas brutas y ticket promedio de órdenes completadas">
      {!salesByDay.length ? <EmptyReport /> : <Box sx={{ height: 310 }}><ResponsiveContainer><AreaChart data={salesByDay} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}><defs><linearGradient id="financeRevenue" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ff4b45" stopOpacity={.25}/><stop offset="95%" stopColor="#ff4b45" stopOpacity={.01}/></linearGradient></defs><CartesianGrid strokeDasharray="3 6" vertical={false} stroke="#edf0f3"/><XAxis dataKey="label" axisLine={false} tickLine={false} minTickGap={25} tick={{ fontSize: 10 }}/><YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10 }}/><Tooltip formatter={(value, name) => [money(value), name]}/><Area type="monotone" dataKey="revenue" name="Ventas" stroke="#ff4b45" strokeWidth={2.7} fill="url(#financeRevenue)"/><Area type="monotone" dataKey="averageTicket" name="Ticket promedio" stroke="#3c4148" strokeWidth={1.8} fill="transparent"/></AreaChart></ResponsiveContainer></Box>}
    </ReportPanel>
    <Stack gap={2}>
