    ...
    <Box sx={{ mb: 2.5 }}>
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2,minmax(0,1fr))" }, gap: 1.25 }}>
        {programs.map((item) => {
          ...
          const available = Number(item.progress?.availableRewards || 0);
          return (
            <Paper key={item.businessId} variant="outlined" sx={{ p: 1.75, borderRadius: "8px" }}>
              <Stack spacing={1}>
                ...
                <Typography variant="caption" color="text.secondary">
                  {item.pausedByPlan
                    ? "Programa pausado. Tu progreso y recompensas se conservan."
                    : available > 0
                      ? "Puedes aplicar una recompensa en el checkout de este negocio."
